...
```
The mean total number of steps is **`r round(meanSteps)`** and the median total number of steps is **`r medianSteps`**.
...
plot(
    as.integer(names(avgStepsPerInterval)), 
    avgStepsPerInterval, 
    type="l", 
    xlab="Time as 5-minute intervals", 
...
```
The interval, **`r maxInterval`**, corresponding to **`r result`**, contained **`r round(maxSteps)`**, the maximum average number of steps in any interval. 
...
```
The total number of missing rows is **`r missingrows`**.
...
```
The median number of steps is **`r medianStepsI`** unchanged from **`r medianSteps`** after replacing missing values whereas the mean number of steps is **`r round(meanStepsI)`** has increased from **`r round(meanSteps)`** after replacing missing values.
...
impute$wk <- ifelse((weekdays(as.Date(impute$date)) %in% c('Saturday','Sunday')),"weekend","weekday")
